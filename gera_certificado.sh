...
echo "Escreva nome do dominio ex: galpsds-q.galp.com ?"
read DOM
mkdir $DOM
cd $DOM
...
#######################################
#STEP1
openssl pkcs12 -in /certificados/pfx/$DOM.pfx -nocerts -out privatekey.pem
...
#STEP3
openssl pkcs12 -in /certificados/pfx/$DOM.pfx -clcerts -nokeys -out Certificate_body_cert-file.pem
...
#STEP4
openssl pkcs12 -in /certificados/pfx/$DOM.pfx -cacerts -nokeys -chain -out Certificate_chain_ca-chain.pem